...
const Navbar = () => {
    ...
    return (
        <div className = 'bg-white flex items-center justify-center p-6 w-screen sticky' >
            ...
            <div className='ml-32 bg-zinc-100 flex items-center border border-spacing-1 p-2 w-6/12 rounded-lg'>
                <img src={icnSearch} className='w-5 h-5'/>
                <input type="text" placeholder='Search for news' className='ml-4 bg-zinc-100' />
            </div>
            <button onClick={googleSignin} className='ml-44 bg-blue-600 text-white p-2 w-28 rounded-md'>{user ? 'Sign Out' : 'Sign In'}</button>
        </div >
    );
}
